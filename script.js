...
function createTouchHeart(x, y) {
  ...
  heart.style.setProperty("--size", `${18 + Math.random() * 14}px`);
  heart.style.setProperty("--x-drift", `${(Math.random() * 2 - 1) * 42}px`);
  heart.style.setProperty("--y-rise", `${70 + Math.random() * 55}px`);
  heart.style.setProperty("--spin", `${(Math.random() * 2 - 1) * 22}deg`);
  els.heartLayer.appendChild(heart);
...
function setupCountdownTouchHearts() {
  const emit = (event) => {
    ...
    // Keep the countdown fully locked, but make every touch feel magical.
    createTouchHeart(x, y);
    createTouchHeart(x + (Math.random() * 16 - 8), y - (Math.random() * 10));
  };

  els.countdownScreen.addEventListener("pointerdown", emit, { passive: true });
  els.countdownScreen.addEventListener("touchstart", emit, { passive: true });
}
