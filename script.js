...
function setupCountdownTouchHearts() {
  ...
  const emit = (x, y) => {
    if (isUnlocked() || els.countdownScreen.classList.contains("is-hidden")) return;
    if (!Number.isFinite(x) || !Number.isFinite(y)) return;

    // Keep the countdown locked, but make every tap/click feel magical.
    // A fuller burst: many small hearts appear together, then rise slowly.
    const heartCount = 16 + Math.floor(Math.random() * 7);
    for (let i = 0; i < heartCount; i++) {
      createTouchHeart(x, y, i);
    }
  };
  ...
  const onTouchStart = (event) => {
    // Avoid duplicate hearts on browsers that fire both touch and pointer events.
  ...
  const onClick = (event) => {
    // Keep click as a reliable fallback for desktop browsers and older engines.
    if (Date.now() - lastPointerStamp < 350) return;
    emit(event.clientX, event.clientY);
  };

  if ("PointerEvent" in window) {
    els.countdownScreen.addEventListener("pointerdown", onPointerDown, { passive: true });
  } else {
    els.countdownScreen.addEventListener("touchstart", onTouchStart, { passive: true });
    els.countdownScreen.addEventListener("mousedown", onClick, { passive: true });
  }

  els.countdownScreen.addEventListener("touchstart", onTouchStart, { passive: true });
  els.countdownScreen.addEventListener("click", onClick, { passive: true });
}
